...
theorem permCirc'_complete : ∀i, ∃ w, Semantics.Witgen.runWithInputs permCirc' i = some w ∧
    (sha256CS.2.satisfies (w[·]!)) := by
  intro i
  apply Complete.adequate <;> sorry

theorem permCirc'_sound (inp : Vector Bool 768) (wit : Nat → Bool)
    (hWit: ∀ (i : Fin 768), inp[i] = wit i.val):
    sha256CS.2.satisfies wit → sha256CS.1.map (fun i => i.eval wit) = perm' inp := by
  intro hp
  apply Sound.adequate (circ := permCirc') (P := fun i o => o.map (·.eval wit) = perm' inp) <;> sorry
