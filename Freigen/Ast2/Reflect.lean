...
def Reflection.ret {H : Signature} {Φ : Prop} {α : Tp}
    (v : α.denote (ITree2.CompE H.spec))
    {v' : α.denote (ITree2.CompE H.spec)} (h : Φ → v = v') :
...
def Reflection.bin {H : Signature} {Φ : Prop} {a b c : Tp0} {α : Tp}
    ...
    {m : Freek H.spec (α.denote (ITree2.CompE H.spec))}
...
def Reflection.op {H : Signature} {Φ : Prop} {α : Tp}
    ...
    {ks : (H.output e).denote →
      Freek H.spec (α.denote (ITree2.CompE H.spec))}
  ...
  ⟨.op e i (fun b x => (blocks b x).1) (fun o => (k o).1), fun hΦ => by
    ...
    congr 1
    · apply congrArg (fun bs : ((bx : H.spec.Block e) →
          ITree2.CompE H.spec (H.spec.branchOutput e bx.1)) =>
          ITree2.CompE.op (H := H.spec)
