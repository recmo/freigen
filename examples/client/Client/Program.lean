...
/-- My own circuit program, written against Freigen's higher-order `Free Source`:
    hint `x = 15`, hint `y = 2·x`, then constrain `y == 30`. -/
def myProgram : Circuit Unit := do
  let x ← Circuit.hint (pure (10 + 5))
  let y ← Circuit.hint (pure (x * 2))
  Circuit.assert (y == 30)
